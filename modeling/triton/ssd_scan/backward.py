...
def chunk_scan_chunk_state_backward_u(
    ...
    chunk_scan_chunk_state_backward_u_kernel[grid](
        u, delta, decay_cumsum, B, CB, y_grad, h_grad, u_grad, delta_grad,
...
def chunk_state_backward_B(
    ...
    chunk_state_backward_B_kernel[grid](
        u, B, delta, decay_cumsum, h_grad, decay_grad_from_B, B_grad,
...
def chunk_scan_backward_C(
    ...
    chunk_scan_backward_C_grad_kernel[grid](
        C, h, decay_cumsum, y_grad, decay_grad_from_C, C_grad,
...
def chunk_scan_backward_CB(
    ...
    chunk_scan_backward_CB_kernel[grid](
        u, delta, decay_cumsum, y_grad, CB_grad,
...
def chunk_scan_backward_decay_cumsum(
    ...
    chunk_scan_backward_decay_cumsum_kernel[grid](
        u, delta, decay_cumsum, y_grad, CB, decay_grad,
...
def chunk_cumsum_backward(
    ...
    chunk_cumsum_backward_kernel[grid](
        A, delta_raw, delta_bias, delta_grad, decay_grad_total, delta_bias_grad, delta_raw_grad, A_grad,
...
def ssd_scan_backward(
    ...
    decay_cumsum, delta = chunk_cumsum_forward(A, delta_raw, delta_bias, chunk_size, use_delta_softplus, delta_limit)
    ...
    h_grad = chunk_scan_backward_h(C, decay_cumsum, y_grad)
    ...
    u_grad, delta_grad = chunk_scan_chunk_state_backward_u(u, delta, decay_cumsum, B, CB, y_grad, h_grad)

    B_grad, decay_grad_from_B = chunk_state_backward_B(u, B, delta, decay_cumsum, h_grad)

    C_grad, decay_grad_from_C = chunk_scan_backward_C(C, h, decay_cumsum, y_grad)

    CB_grad = chunk_scan_backward_CB(u, delta, decay_cumsum, y_grad, num_groups=num_groups)

    B_grad = bmm_chunk_backward(C, CB_grad, B_grad)
    C_grad = bmm_chunk_backward(B, CB_grad.transpose(-1, -2).contiguous(), C_grad)

    decay_grad_from_C[..., -1] += decay_last_grad
    decay_grad_prev_cumsum = decay_grad_from_C.flip([-1]).cumsum(dim=-1).flip([-1])

    decay_grad = chunk_scan_backward_decay_cumsum(u, delta, decay_cumsum, y_grad, CB)
    ...
    A_grad, delta_raw_grad, delta_bias_grad = chunk_cumsum_backward(
        A, delta_raw, delta_bias, delta_grad, decay_grad_total, 
